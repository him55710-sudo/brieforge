...
import { emptyAnswers, fields, generateSchema, getQuestions, PRESET_IDEA, presetAnswers, questionListSchema, savedSchema, STORAGE_KEY, synthesizeBrief, type Answers, type Mode, type Question, type SavedBrief } from "@/lib/brief";
...
export default function BriefForge() {
  ...
  async function start(usePreset = false) {
    ...
    setIdea(value); setPreset(usePreset); setAnswers(usePreset ? { ...presetAnswers } : { ...emptyAnswers });
  ...
  async function generate() {
    if (actionLock.current) return;
    const parsed = generateSchema.safeParse({ idea, preset, answers, localOnly });
    if (!parsed.success) { setError("7개 답변을 모두 입력해 주세요."); return; }
    actionLock.current = true; setBusy(true); setError(""); setNotice("");
    ...
    const result: SavedBrief = { version: 1, idea, preset, answers: { ...answers }, markdown, mode, savedAt: new Date().toISOString() };
  ...
  function restore() {
    if (!saved) return;
    setBrief(saved); setIdea(saved.idea); setPreset(saved.preset); setAnswers(saved.answers);
    setQuestions(getQuestions(saved.preset)); setNotice(""); setError(""); setStage("result"); setRaw(false);
  }
  function reset() {
    setStage("landing"); setIdea(""); setPreset(false); setAnswers({ ...emptyAnswers }); setError(""); setNotice(""); setBrief(null);
  }
  async function copy() {
    if (!brief) return;
