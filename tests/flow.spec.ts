import { fields, setupTools } from "../lib/brief";
...
test("preset survey → brief → copy/download → restore → delete", async ({ page, context }) => {
  ...
  for (let i = 0; i < fields.length; i++) {
    await expect(page.getByText("왜 이 질문을 하나요?", { exact: true })).toBeVisible();
    await page.getByRole("button", { name: "다음 질문" }).click();
  }
  await expect(page.getByText("왜 이 질문을 하나요?", { exact: true })).toBeVisible();
  await page.getByRole("group", { name: "Codex 구독 / 이용 준비", exact: true }).getByLabel("준비됨", { exact: true }).check();
  ...
  await expect(page.locator("article")).toContainText("Codex 구독 / 이용 준비 — 준비됨");
  await expect(page.locator("article")).toContainText("개발 시작 전 준비물");
...
test("mobile generic flow works with backend unreachable, supports editing and recent restore", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "마스터 브리프 생성" })).toBeDisabled();
  for (const [i, tool] of setupTools.entries()) {
    await page.getByRole("group", { name: tool.name, exact: true }).getByLabel(i === 0 ? "준비됨" : i === 1 ? "아직" : "이번엔 불필요", { exact: true }).check();
  }
  await page.screenshot({ path: "/tmp/briefforge-setup-mobile.png", fullPage: true });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
  await page.getByRole("button", { name: "이전", exact: true }).click();
  await expect(page.getByLabel("나의 답변")).toHaveValue(`사용자가 직접 정한 요구사항 ${fields.length}`);
  await page.getByRole("button", { name: "다음 질문" }).click();
  await expect(page.getByRole("group", { name: setupTools[0].name, exact: true }).getByLabel("준비됨", { exact: true })).toBeChecked();
  await page.getByRole("button", { name: "마스터 브리프 생성" }).click();
  await expect(page.locator("article")).toContainText("사용자가 직접 정한 요구사항 7");
  await expect(page.locator("article")).toContainText("Claude 구독 / 이용 준비 — 아직");
